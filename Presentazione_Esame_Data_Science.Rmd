---
title: "Progetto Esame Advanced Data Science"
author: "Devin mancardi 159108"
date: "2026-01-17"
output: 
...
ggraph(g_final_ALL, layout = "fr") +
  ...
  labs(title = "C. elegans Connectome",
       subtitle = "Rete completa con distinzione dei neuroni Ibridi (Sensorimotor) e sistema nervoso faringeo",
       fill = "Ruolo",  
       edge_color = "Tipo Connessione")

```

Come si può notare ho voluto identificare i nueoroni che sia erano motori (ovvero davano un input al muscolo) che sensoriali (ricevono un stimolo), andando a creare Il tipo "Sensorimotor". 
Questo perchè erano presenti in entrambi i file *Muscles* e *Sensory*
...
g_marked <- g_final_ALL %>%
  activate(nodes) %>%
  mutate(comp_id = group_components(type = "weak")) # assegno un numero uguale se esiste un percorso che collega i nodi con weak non importa la direzione della freccia
...
ggplot(df_plot, aes(x = densita)) +
  ...
  # Linea rossa: La tua osservazione reale
...
simula_transitivity <- function() {
  # 1. Pesca nodi casuali dal somatico
  nodi_random <- sample(V(g_somatic)$name, n_nodi_faringe)
  
  # 2. Crea sottografo
  sub_g <- induced_subgraph(g_somatic, nodi_random)
  
  # 3. Calcola Transitività
...
# P-VALUE: Quindi contiamo quante volte il Random è MAGGIORE o UGUALE alla Faringe
...
# PARAMETRI DEL FIT (Quelli che hai calcolato tu) 
X_MIN <- 21 
ALPHA <- 3.812  
LAMBDA <- 0.09189 
...
# GENERAZIONE CURVE TEORICHE (Solo da X_MIN in poi) 
...
# layout 
set.seed(123) 
layout_final <- create_layout(g_viz_final, layout = "fr") 
...
# Usiamo SPEARMAN invece di Pearson perché i dati non seguono una distribuzione normale (Power Law)
...
df_complete <- g_somatic %>%
  ...
  mutate(
    ...
    Betweenness = centrality_betweenness(directed = TRUE, normalized = TRUE),
    Closeness   = centrality_closeness(mode = "out", normalized = TRUE), # "out" perché ci interessa quanto velocemente raggiungono gli altri
...
### Rappresentazione visiva dei Cwluster (louvian)
...
```{r, fig.width=19, fig.height=12, out.width="100%"}
...
# Calcoliamo TUTTI i cammini minimi tra tutte le coppie
...
* Clustering ($C \gg C_{rand}$): La rete mostra un coefficiente di clustering enormemente superiore al caso indicando una specializzazione locale estrema.
...
```{r , echo=FALSE, warning = FALSE, fig.width=15, fig.height=8, out.width="100%"}
g_stats <- g_somatic %>%
  activate(nodes) %>%
  mutate(
...
```{r network_plot, echo=FALSE, warning = FALSE, fig.width=15, fig.height=8, out.width="100%"}
...
il grafico Authority vs Hub Score, rappresenta esattemente un sistema biologico. i nodi in alto a destra hanno punteggi altri su entrambi gli assi, ovvero che ricevono informazioni (Authority), le elaborano e le ritrasmettono (Hub). Non sono specializzati, gestiscono l'intero flusso di informazioni
...
# attacchi DINAMICI
# funzione accetta una metrica ("degree", "betweenness", "pagerank") e ricalcola dinamicamente ogni metrica quando si toglie un nodo
...
# Ordino per K-Shell decrescente (Partiamo dal nucleo più denso, es. K=10) poi a parità di guscio, uccidi chi ha più Betweenness
